...
import { DUMMY_DETAIL } from "./constant";
...
const DUMMY_KUPON_PERCENT = 0.1; // 10%

const total = computed(() => DUMMY_DETAIL.harga - diskon.value);
...
function checkKupon() {
  ...
  setTimeout(() => {
    if (kodeKupon.value.trim().toUpperCase() === DUMMY_KUPON_CODE) {
      diskon.value = Math.round(DUMMY_DETAIL.harga * DUMMY_KUPON_PERCENT);
      ...
      <div class="h-fit rounded-xl border border-gray-300 bg-white px-5 py-5 shadow-md md:px-8 lg:col-span-2">
        ...
        <div class="flex flex-col gap-2 border-y border-gray-300 py-4 text-sm font-extralight text-gray-500">
          <div class="flex justify-between gap-4">
            <p>Harga</p>
            <p>{{ formatRupiah(DUMMY_DETAIL.harga) }}</p>
          </div>
          <div class="flex justify-between gap-4">
            <p>Diskon</p>
