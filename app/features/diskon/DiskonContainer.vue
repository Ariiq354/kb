...
const { data, status, refresh } = await useFetch("/api/v1/diskon", {
  query,
});
...
function clickEdit(item: ExtractFetchData<typeof data>[number]) {
  state.value = {
    ...
    kode: item.kode,
    persen: item.persen,
    batasWaktu: parseDate(item.batasWaktu),
    batasPemakai: item.batasPemakai,
    status: item.status,
  };
  modalOpen.value = true;
}
...
async function clickDelete(ids: number[]) {
  openModal(ModalConfirm, { path: "/api/v1/diskon", body: { ids }, refresh });
}
</script>
